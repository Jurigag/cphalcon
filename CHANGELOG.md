...
- Added `Phalcon\Acl\Roleable` and `Phalcon\Acl\Resourceable` Interfaces, Now you can pass objects to `Phalcon\Acl\Adapter\Memory::isAllowed` as `roleName` and `resourceName`, also they will be automatically passed to function defined in `Phalcon\Acl\Adapter\Memory::allow` or `Phalcon\Acl\Adapter\Memory::deny` by type
...
- `Phalcon\Acl\Adapter\Memory::isAllowed` have 4th argument - parameters, you can pass arguments for function defined in `Phalcon\Acl\Adapter\Memory::allow` or `Phalcon\Acl\Adapter\Memory::deny` as associative array where key is argument name
